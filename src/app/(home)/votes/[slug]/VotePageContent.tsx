...
export function VotePageContent({ categorySlug, categories }: Props) {
  ...
  const winningNominations = api.nominations.getWinningNominations.useQuery();

  useEffect(() => {
    if (winningNominations.data && winningNominations.data.length !== 0) {
      router.push("/");
    }
  }, [winningNominations.data, router]);

  if (!data?.currentCategory) return <VotesPageSkeleton />;
